...
During this sprint, we were still not able to make full use of all agile methods. One of the reasons for this was, that not all base modules in our application have been fully implemented thus preventing people from working on certain features. However, we expect the remaining base modules (template engine & testing environment) to be incorporated during the coming sprint. Once the basis for our application is complete, we can implement more features which we haven't been able to work on.
